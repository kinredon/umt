...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, target=False, eta=1.0, binary=False):
        ...
        # compute bbox offset
        if binary:
            bbox_pred = self.RCNN_bbox_pred_target(pooled_feat)
        else:
        ...
        if binary:
            cls_score = self.RCNN_cls_score_target(pooled_feat)
        else:
