...
from model.faster_rcnn.faster_rcnn_global_local_binary import _fasterRCNN
...
class resnet(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.RCNN_cls_score_target = nn.Linear(feat_d, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred = nn.Linear(feat_d, 4)
      self.RCNN_bbox_pred_target = nn.Linear(feat_d, 4)
    else:
      self.RCNN_bbox_pred = nn.Linear(feat_d, 4 * self.n_classes)
      self.RCNN_bbox_pred_target = nn.Linear(feat_d, 4 * self.n_classes)
