...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, target=False, test=False, eta=1.0):
        if test:
            self.training = False
        ...
        base_feat_avgpool = nn.AdaptiveAvgPool2d((7, 7))(base_feat)
        if self.gc:
        ...
        if test:
            self.training = True
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label,d_pixel, domain_p, base_feat_avgpool#,diff
