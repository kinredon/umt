...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None,
                 seg_return=False, path_return=False, augment=False):
        self._roidb = roidb
        ...
        for i in range(num_batch):
            ...
            try:
                ...
                self.ratio_list_batch[left_idx:(right_idx+1)] = torch.tensor(target_ratio.astype(np.float64)) # for kitti, trainset ratio list ,each batch is same number
    ...
    def __getitem__(self, index):
        ...
        blobs = get_minibatch(minibatch_db, self._num_classes, seg_return=self.seg_return, augment=self.augment, seed=index)
