...
class imdb(object):
    ...
    def append_flipped_images(self):
        ...
        for i in range(num_images):
            ...
            #   boxes[k,0]=widths[i]-oldx2[k] - 1 if widths[i]>oldx2[k] else 0
            #   boxes[k,2]=widths[i]-oldx1[k] - 1 if widths[i]>oldx1[k] else 0

            try:
                assert (boxes[:, 2] >= boxes[:, 0]).all()
            except:
                print("error: {}".format(self.image_index[i]))
    ...
    def evaluate_recall(
        ...
        for i in range(self.num_images):
            ...
            gt_inds = np.where(
                (self.roidb[i]["gt_classes"] > 0) & (max_gt_overlaps == 1)
            )[0]
